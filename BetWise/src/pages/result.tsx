    ...
    return (
        <section className="result-bar">
            <div className="result-bar-container">
                <div
                    className="result-segment win"
                    style={{ width: `${winPercent}%` }}
                >
                    {winPercent > 5 && <span className="percent-label">{formatPercent(winPercent)}</span>}
                </div>
                <div
                    className="result-segment draw"
                    style={{ width: `${drawPercent}%` }}
                >
                    {drawPercent > 5 && <span className="percent-label">{formatPercent(drawPercent)}</span>}
                </div>
                <div
                    className="result-segment loss"
                    style={{ width: `${lossPercent}%` }}
                >
                    {lossPercent > 5 && <span className="percent-label">{formatPercent(lossPercent)}</span>}
                </div>
            </div>
        </section>
    );
